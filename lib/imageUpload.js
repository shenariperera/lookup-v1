 ...
 * @param {string} type - Image type: 'logo', 'offer', 'banner', 'company'
 * @returns {Promise<File>} - Optimized image file
 */
export async function optimizeImage(file, type = 'offer') {
  ...
  const preset = OPTIMIZATION_PRESETS[type] || OPTIMIZATION_PRESETS.offer;
 ...
 * @param {string} folder - Folder path in R2 bucket (e.g., 'company-logos', 'offer-images')
 ...
 * @returns {Promise<string>} - Public URL of uploaded image
 */
export async function optimizeAndUpload(file, type = 'offer', folder = 'images') {
...
export const optimizeDealImage = (file) => optimizeImage(file, 'offer');
